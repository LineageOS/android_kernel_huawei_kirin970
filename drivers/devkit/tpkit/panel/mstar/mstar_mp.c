...
void msg30xx_uniformity_calc_data(int *d_data, int d_size, int *c_buf, MpUniformityPart uni_cont)
{
   ...
   for(i = 0, index = 0; i < sense_num ; i++)
    ...
    for(i = 0, index = 0; i <sense_num ; i++)
    {
        for(j = 0; j <drv_num ; j++, index ++)
        {
            TS_LOG_DEBUG("%d,",delta_LR_buf[index]);
        }
    }
    ...
    for(i = 0, index = 0; i <sense_num ; i++)
    {
        for(j = 0; j <drv_num ; j++, index ++)
        {
            TS_LOG_DEBUG("%d,",delta_UD_buf[index]);
        }
    }
}
...
int msg30xx_uniformity_judge(int *deltac_data, int deltac_size)
{
    ...
    TS_LOG_DEBUG("%s: deltac_data_buf\n",__func__);
    for(i = 0, index = 0; i <sense_num ; i++)
    {
        for(j = 0; j <drv_num ; j++, index ++)
        {
            TS_LOG_DEBUG("%d,",(0-deltac_data[index]));
        }
    }
    ...
    for(i = 0, index = 0; i <6 ; i++)
    {
        for(j = 0; j <sense_num ; j++, index ++)
        {
            TS_LOG_DEBUG("%d,",border_ratio_buf[i*sense_num+j]);
        }
    }
...
static int mp_main_init_var(void)
{
    ...
    if(ERR_ALLOC_MEM(mp_test_result->puniformity_deltac) || ERR_ALLOC_MEM(mp_test_result->pdelta_LR_buf) || ERR_ALLOC_MEM(mp_test_result->pdelta_UD_buf) || ERR_ALLOC_MEM(mp_test_result->pborder_ratio_buf))
    {
