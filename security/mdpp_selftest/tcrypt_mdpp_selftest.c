/*
 * kenel crypto mdpp sleftest module.
 *
 * tcrypt_mdpp_selftest.c
 *
 * the tcrypt_mdpp_selftest.c - execute kenel mdpp sleftest
 *
 * Copyright (c) 2017-2027, Huawei Tech. Co., Ltd. All rights reserved.
 */
...
static ssize_t tcrypt_result_show(struct device *dev, struct device_attribute *attr,
                        char *buf);
...
static ssize_t tcrypt_result_show(struct device *pdev, struct device_attribute *attr,
			   char *buf)
{
    ...
    case TCRYPT_START_SELFTEST:
        {
            ...
            latest_result = do_alg_tests();
            break;
        }
    case TCRYPT_READ_RESULT:
        {
            if (copy_to_user((int *)arg, &latest_result, sizeof(latest_result)))
                ret = -EFAULT;
